...
class FilteredReportsScreen extends StatefulWidget {
  final String status;
  
  const FilteredReportsScreen({super.key, required this.status});

  @override
  State<FilteredReportsScreen> createState() => _FilteredReportsScreenState();
}

class _FilteredReportsScreenState extends State<FilteredReportsScreen> {
  @override
  void initState() {
    super.initState();
    ...
    return Scaffold(
      ...
      body: Consumer<ReportProvider>(
        builder: (context, reportProvider, child) {
          ...
          final filteredReports = reportProvider.userReports
              .where((report) => report.status.toLowerCase() == widget.status.toLowerCase())
              .toList();
            ...
            return Center(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 20),
                  Text(
                    'No ${widget.status} Reports',
                    style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold),
                  ),
                  const SizedBox(height: 10),
                  Text(
                    'You have no reports with ${widget.status} status.',
